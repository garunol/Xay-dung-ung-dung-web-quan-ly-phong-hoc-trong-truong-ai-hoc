...
function handleCreateSubject () {
    ...
    if (!isset($_POST['subject_code']) || !isset($_POST['subject_name'])) {
        header("Location: ../views/teacher/create_subject.php?error=Thiếu thông tin cần thiết");
        exit();
    }
    ...
    if (empty($subject_code) || empty($subject_name)) {
        header("Location: ../views/teacher/create_subject.php?error=Vui lòng điền đầy đủ thông tin");
        exit();
    }
    ...
    if ($result) {
        header("Location: ../views/teacher.php?success=Thêm học phần thành công");
    } else {
        header("Location: ../views/teacher/create_subject.php?error=Có lỗi xảy ra khi thêm học phần");
    }
    exit();
}
...
function handleEditSubject() {
    ...
    if ($result) {
        header("Location: ../views/teacher.php?success=Cập nhật học phần thành công");
    } else {
...
function handleDeleteSubject() {
    ...
    if (!isset($_GET['id']) || empty($_GET['id'])) {
        header("Location: ../views/teacher.php?error=Không tìm thấy ID học phần");
        exit();
    }
    ...
    // Validate ID là số
    if (!is_numeric($id)) {
        header("Location: ../views/teacher.php?error=ID học phần không hợp lệ");
        exit();
    }
    ...
    if ($result) {
        header("Location: ../views/teacher.php?success=Xóa học phần thành công");
    } else {
        header("Location: ../views/teacher.php?error=Xóa học phần thất bại");
    }
    exit();
}
?>
